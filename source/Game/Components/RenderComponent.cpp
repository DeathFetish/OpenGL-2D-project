...
void RenderComponent::render(const bool isFlipped)
{
	if (!renderEnable)
		return;
...
void RenderComponent::addAnimation(const std::string name, SpriteAnimation& animation) { animations.insert(std::make_pair(name, animation)); }
void RenderComponent::deleteAnimation(const std::string name) { animations.erase(name); }

void RenderComponent::setCurrentAnimation(const std::string& animationName, const unsigned int frameId)
{
...
void RenderComponent::setParametrs(const rapidjson::Value& jsonObject)
{
